...
Three feature enrichment strategies yield cumulative gains beyond the case-type baseline. \textit{History features} encode each section's expanding-window trajectory (case frequencies, flip rate, volatility), adding 0.5pp. \textit{Spatial features} capture neighborhood context via administrative hierarchy (mean neighbor changes, case distribution, price gap), adding 1.0pp. \textit{Mean-reversion features} exploit the anti-persistent structure of price changes (rent autocorrelation $= -0.26$, sale $= -0.21$) by encoding gap-to-rolling-mean, expanding z-scores, and shock magnitude, adding 0.7pp. \textit{Rolling-origin retraining}, where the model is refit each test quarter using all data prior to that quarter, adds 2.3pp by adapting to distributional shift (including COVID-19). \textit{Regression meta-features}, which are out-of-time XGBoost regression forecasts of next-quarter rent and sale price changes trained on expanding annual folds, add 0.5pp. Finally, \textit{directional decomposition} replaces the direct five-class classifier with two three-class models predicting price direction (down, zero, up) for rent and sale separately; the five gentrification classes are then recovered by multiplying the constituent probabilities (e.g., $P(\text{A}) = P(\text{rent up}) \times P(\text{sale} \leq 0)$), adding 0.2pp. Per-class precision and recall are balanced (0.43--0.55 range); Case B achieves the highest recall (0.55).
